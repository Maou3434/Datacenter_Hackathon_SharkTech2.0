...
export function calculateSuitability(
  ...
  const suitability = (
    (location.scores.environmental * weights.environmental) +
    (location.scores.population * weights.population) +
    (location.scores.renewable * weights.renewable) +
    (location.scores.water * weights.water) +
    (location.scores.terrain * weights.terrain) +
    (location.scores.infrastructure * weights.infrastructure)
  ) / (totalWeight * 100);
